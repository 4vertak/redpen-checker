...
3. Система распознаёт рукописный текст (Yandex Vision API) и анализирует ошибки (DeepSeek API).
...
Веб-приложение на **Go + Gin** работает как оркестратор: получает фото → отправляет в Yandex Vision API → распознанный текст → отправляет в DeepSeek API → возвращает учителю результат с ошибками и пояснениями. Всё взаимодействие в MVP — синхронное.
