...
- **«Объяснить задание»** — принимает фото формулировки, возвращает краткое объяснение.
- **«Проверить готовое»** — принимает фото решения, опционально — эталонный ответ. Возвращает список ошибок с пояснениями.

## Обработка ошибок OCR

- Если текст не распознан (плохое освещение, неразборчивый почерк), система возвращает понятное сообщение: «Не удалось распознать текст. Попробуйте переснять страницу при хорошем освещении».
- Если изображение не содержит текста (например, загружено фото кота), система возвращает сообщение: «Не удалось найти текст на изображении. Убедитесь, что вы загрузили фото тетради или учебника».

## Конфиденциальность

- Загруженные фото обрабатываются и сразу удаляются. В MVP используется временное хранение на сервере.
